...
class IrreducibleGraphSplitter {
    ...
    private void handleStronglyConnectedComponent(int top, int[] scc) {
        ...
        int nodesToCopyCount = 0;
        for (int node : scc) {
            if (node != domain) {
                nodesToCopyCount += realNodes[node].size();
            }
        }
        int[] realNodesToCopy = new int[nodesToCopyCount];
        ...
        for (int successor : cfg.outgoingEdges(top)) {
            int subgraphSuccessor = map[successor];
            if (subgraphSuccessor >= 0) {
                subgraph.addEdge(0, subgraphSuccessor);
            }
        }
        for (int node : scc) {
            ...
            for (int successor : successors) {
                int subgraphSuccessorCopy = map[successor];
                ...
                int subgraphSuccessor = map[subgraphNode];
                if (subgraphSuccessor >= 0) {
                    if (subgraphNodeCopy >= 0) {
